...
const App: React.FC = () => {
  ...
  // ETAT INITIAL AVEC LAT/LNG PAR DÉFAUT (Pour éviter crash carte)
  ...
  const handleLogout = () => {
    Alert.alert("Déconnexion", "Quitter l'application ?", [
      ...
      { text: "Quitter", style: "destructive", onPress: () => {
          ...
          setView('login'); setPeers({}); setPings([]); setHostId('');
  ...
  const handleData = useCallback((data: any, fromId: string) => {
    switch (data.type) {
      ...
      case 'SILENCE':
        ...
        break;
      case 'PRIVATE_REQ':
        Alert.alert("Appel Privé", `Demande de ${fromId}`, [
            { text: "Refuser", style: "cancel" },
            { text: "Accepter", onPress: () => showToast("Mode Privé (Bientôt)") }
        ]);
        break;
    }
  }, [showToast]);
  ...
  const startServices = async () => {
    ...
    // DEMANDE PERMISSION CAMERA (CRITIQUE)
    if (!permission?.granted) {
        const camPerm = await requestPermission();
        if(!camPerm.granted) showToast("Caméra refusée", "error");
    }
