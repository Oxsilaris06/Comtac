...
// Style JSON pour cacher TOUT le fond Google Maps par défaut
  ...
  const renderMarker = (u: UserData, isMe: boolean) => {
    ...
    return (
      <Marker
        ...
        anchor={{ x: 0.5, y: 0.5 }} flat={true} rotation={u.head || 0} zIndex={isMe ? 150 : 50}
      >
  ...
  return (
    <View style={styles.container}>
      <MapView
        ...
        mapType="standard" // FIX: On force standard pour initialiser le moteur
        customMapStyle={BLANK_MAP_STYLE} // FIX: On cache tout manuellement
      ...
      >
        <UrlTile
          ...
          maximumZ={19} flipY={false} zIndex={100} tileSize={256}
        ...
        {showTrails && Object.entries(trails).map(([id, coords]) => {
           ...
           return (<Polyline key={`trail-${id}`} coordinates={coords} strokeColor={color} strokeWidth={isMe ? 3 : 2} lineDashPattern={isMe ? [0] : [5, 5]} zIndex={110} />);
        })}
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#212121' }, // Fond gris foncé par défaut
